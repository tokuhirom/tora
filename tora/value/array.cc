...
struct ArrayIteratorImpl {
  ...
  ArrayIteratorImpl(Value* v) : counter(0), parent(v) {}
};
...
void tora::array_iter_next(Value* iter) { ary_iter(iter)->counter++; }
...
tra_int tora::array_size(const Value* self) { return ary(self)->size(); }

void tora::array_push_front(Value* self, Value* v) { ary(self)->push_front(v); }

void tora::array_push_back(Value* self, Value* v) { ary(self)->push_back(v); }

void tora::array_pop_back(Value* self) { ary(self)->pop_back(); }

void tora::array_pop_front(Value* self) { ary(self)->pop_front(); }
...
Value* tora::array_back(const Value* self) { return ary(self)->back().get(); }
...
tra_int tora::tuple_size(const Value* self) { return tuple(self)->size(); }

void tora::tuple_push(Value* self, Value* v) { tuple(self)->push_back(v); }
