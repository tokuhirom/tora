...
std::string ClassValue::name() const {
  return VAL().vm_->symbol_table->id2name(VAL().name_id_);
}

void ClassValue::add_method(const SharedPtr<Value>& code) {
  VAL().methods_.insert(std::map<ID, SharedPtr<Value>>::value_type(
      code->upcast<CodeValue>()->func_name_id(), code));
}

void ClassValue::add_method(ID name_id, const CallbackFunction* cb) {
  VAL().methods_.insert(std::map<ID, SharedPtr<Value>>::value_type(
      name_id, new CodeValue(this->name_id(), name_id, cb)));
}
...
SharedValue ClassValue::get_method_list() const {
  MortalArrayValue av;
  for (auto iter = VAL().methods_.begin(); iter != VAL().methods_.end();
       ++iter) {
