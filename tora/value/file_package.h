...
class MortalFilePackageValue : public MortalValue {
  static Value* new_file_package_value(
      ID name_id, const std::shared_ptr<std::map<ID, SharedValue>>& src);

 public:
  MortalFilePackageValue(ID name_id,
                         const std::shared_ptr<std::map<ID, SharedValue>>& src)
