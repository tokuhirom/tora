...
struct ParserState {
  ...
  ParserState(const std::string &filename_)
      : errors(0), failure(false), filename(filename_) {}
  ~ParserState() {}
};
...
class Scanner {
 ...
 public:
  void increment_line_number() { m_lineno++; }
  ...
  */
  ...
  std::string text() { return std::string(m_token, m_token + length()); }
  int length() { return (m_cursor - m_token); }
  int lineno() { return m_lineno; }

  bool in_heredoc() { return heredoc_queue.size() > 0; }
