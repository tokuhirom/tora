...
void tora::Compiler::set_variable(std::string &varname) {
  {
    ...
    if (pos != std::string::npos) {
      ...
      push_op(new OP(OP_SET_PACKAGE_VARIABLE,
                     symbol_table->get_id(std::string("$") + detail)));
      return;
    }
  }
  ...
  int funcdef_level;
  int no =
      this->find_localvar(varname, level, need_closure, is_arg, funcdef_level);
  if (no < 0) {
...
bool tora::Compiler::is_builtin(const std::string &s) {
  const char *bs[] = {
      "say",     "__PACKAGE__", "p",       "require", "eval",   "do",
      "print",   "self",        "opendir", "typeof",  "rand",   "open",
      "exit",    "dump_stack",  "caller",  "callee",  "getcwd", "getpid",
      "getppid", "system",      "abs",     "atan2",   "cos",    "exp",
      "hex",     "int",         "log",     "oct",     "rand",   "sin",
      "sqrt",    "srand",       "sprintf", "printf",  NULL};
  for (int i = 0; bs[i]; i++) {
    if (s == bs[i]) {
      return true;
    }
...
void tora::Compiler::compile(const SharedPtr<Node> &node) {
  ...
  switch (node->type) {
    ...
    case NODE_LOCAL: {
      if (node->at(0)->type != NODE_GET_PACKAGE_VARIABLE &&
          node->at(0)->type != NODE_GETVARIABLE) {
      ...
      */
      ...
      if (funcdef_node->params() && funcdef_node->params()->size() > 0) {
        for (size_t i = 0; i < funcdef_node->params()->size(); i++) {
          params->push_back(
              funcdef_node->params()->at(i)->upcast<StrNode>()->str_value);
          this->define_localvar(std::string(
              funcdef_node->params()->at(i)->upcast<StrNode>()->str_value));
        }
      } else {
      ...
      */
      ...
      if (funcdef_node->have_params()) {
        ...
        for (size_t i = 0; i < funcdef_node->params()->size(); i++) {
          ...
          const SharedPtr<Node> &param_name =
              funcdef_node->params()->at(i)->at(0);

          params->push_back(param_name->upcast<StrNode>()->str_value);
          this->define_localvar(
              std::string(param_name->upcast<StrNode>()->str_value));
        }
      } else {
        // 'sub foo { }' form.
        // params->push_back(std::string("$_"));
      ...
      if (funcdef_node->have_params()) {
        ...
        for (size_t i = 0; i < funcdef_node->params()->size(); i++) {
          assert(funcdef_node->params()->at(i)->children().size() == 2);
          const SharedPtr<Node> &default_node =
              funcdef_node->params()->at(i)->at(1);
          if (default_node.get()) {
...
// push variables  to stack.
    ...
    case NODE_BYTES:
      {
        MortalBytesValue sv(node->upcast<StrNode>()->str_value);
        SharedPtr<ValueOP> tmp = new ValueOP(OP_PUSH_VALUE, sv.get());
        push_op(tmp);
        break;
      }
    case NODE_REGEXP: {
      auto re = node->upcast<RegexpNode>();
      MortalRegexpValue sv(re->regexp_value, re->flags);
      if (!regexp_ok(sv.get())) {
        this->fail("Regexp compilation failed: /%s/ : %s\n",
                   regexp_pattern(sv.get()).c_str(), regexp_error(sv.get()).c_str());
        break;
      }
    ...
    case NODE_FUNCALL: {
      ...
      for (auto iter = args->children().rbegin();
           iter != args->children().rend(); iter++) {
        this->compile(*iter);
      }
        ...
        */
        ...
        push_op(new OP(OP_FUNCALL, args_len,
                       splitted ? symbol_table->get_id(pkgname)
                                : symbol_table->get_id(this->package_)));
      }
      break;
    }
#define C_OP_BINARY(type)                               \
  {                                                     \
    ...
    case NODE_NE:
      C_OP_BINARY(OP_NE);
    case NODE_BITAND:
      C_OP_BINARY(OP_BITAND);
    case NODE_BITOR:
      C_OP_BINARY(OP_BITOR);
    case NODE_BITXOR:
      C_OP_BINARY(OP_BITXOR);
    case NODE_BITLSHIFT:
      C_OP_BINARY(OP_BITLSHIFT);
    case NODE_BITRSHIFT:
      C_OP_BINARY(OP_BITRSHIFT);
#undef C_OP_BINARY
    ...
    case NODE_GETVARIABLE: {
      ...
      if (global >= 0) {
        ...
        int funcdef_level;
        int no =
            this->find_localvar(std::string(node->upcast<StrNode>()->str_value),
                                level, need_closure, is_arg, funcdef_level);
        // dump_localvars();
        ...
        if (1 && need_closure) {
          ...
          if (is_arg) {
            ...
            tmp->operand.int_value =
                (((level) & 0x0000ffff) << 16) | (no & 0x0000ffff);
            push_op(tmp);
          }
        }
      }
...
#define BINARY_ASSIGN(ccc, nnn)                                     \
  case ccc: {                                                       \
    SharedPtr<BinaryNode> r =                                       \
        new BinaryNode(nnn, node->upcast<BinaryNode>()->left(),     \
                       node->upcast<BinaryNode>()->right());        \
    SharedPtr<BinaryNode> p = new BinaryNode(                       \
        NODE_SETVARIABLE, &(*(node->upcast<BinaryNode>()->left())), \
        &(*(r->upcast<Node>())));                                   \
    this->compile(p);                                               \
    break;                                                          \
  }
...
#undef BINARY_ASSIGN
    ...
    case NODE_FILE_TEST: {
      ...
      op->operand.int_value =
          node->upcast<BinaryNode>()->left()->upcast<IntNode>()->int_value;
      this->compile(node->upcast<BinaryNode>()->right());
      push_op(op);
      break;
    }
    ...
    case NODE_CLASS: {
      ...
      if (n->parent()) {
        ...
        op->operand.int_value =
            symbol_table->get_id(n->parent()->upcast<StrNode>()->str_value);
        push_op(op);
      }
