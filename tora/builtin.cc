...
static SharedPtr<Value> builtin_exit(VM *vm, Value *v) { exit(v->to_int()); }
...
static SharedPtr<Value> builtin_say(VM *vm,
 ...
 */
static SharedPtr<Value> builtin_open(
    VM *vm, const std::vector<SharedPtr<Value>> &args) {
  if (args.size() != 1 && args.size() != 2) {
    throw new ExceptionValue(
        "Invalid argument count for open(): %ld. open() function requires 1 or "
        "2 argument.",
        (long int)args.size());
  }
  return File_open(vm, args.at(0).get(),
                   args.size() == 2 ? args.at(1).get() : NULL);
}

static SharedPtr<Value> builtin_print(
    VM *vm, const std::vector<SharedPtr<Value>> &args) {
  auto iter = args.begin();
  for (; iter != args.end(); iter++) {
    SharedPtr<Value> v(*iter);
 ...
 */
static SharedPtr<Value> builtin_rand(
    VM *vm, const std::vector<SharedPtr<Value>> &args) {
  if (args.size() == 0) {
...
static SharedPtr<Value> builtin_self(VM *vm) { return vm->get_self(); }
...
static SharedPtr<Value> builtin_callee(VM *vm) {
  for (auto iter = vm->frame_stack->rbegin(); iter != vm->frame_stack->rend();
       ++iter) {
 ...
 *
 */
static SharedPtr<Value> builtin_caller(
    VM *vm, const std::vector<SharedPtr<Value>> &args) {
  if (args.size() == 0) {
...
static SharedPtr<Value> builtin_sprintf(
    VM *vm, const std::vector<SharedPtr<Value>> &args) {
