 ...
 */
static SharedPtr<Value> file_open_method(
  ...
  return File_open(vm, args.at(1).get(),
                   args.size() == 3 ? args.at(2).get() : NULL);
}
...
static SharedPtr<Value> file_write(VM* vm, Value* self, Value* str_v) {
  ...
  if (ret == str.size()) {
    ...
    throw new ExceptionValue("Cannot write a text for file(%ld < %ld).",
                             (long int)ret, (long int)str.size());
  }
}
 ...
 */
static SharedPtr<Value> file_seek(VM* vm, Value* self, Value* offset,
                                  Value* whence) {
